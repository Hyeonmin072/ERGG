...
CREATE TYPE matching_team_mode_type AS ENUM ('1', '2', '3'); -- 1=솔로, 2=듀오, 3=스쿼드
...
CREATE TABLE IF NOT EXISTS games (
    ...
    matching_team_mode          SMALLINT    NOT NULL,   -- 1=솔로, 2=듀오, 3=스쿼드
    server_name                 VARCHAR(20) DEFAULT 'Asia',
...
CREATE TABLE IF NOT EXISTS game_details (
    ...
    user_num        BIGINT      NOT NULL REFERENCES players(user_num),
    ...
    CONSTRAINT uq_game_user UNIQUE (game_id, user_num)
);
...
CREATE INDEX IF NOT EXISTS idx_gd_user_game     ON game_details (user_num, game_id);
...
CREATE TABLE IF NOT EXISTS octagon_scores (
    id              BIGSERIAL   PRIMARY KEY,
    user_num        BIGINT      NOT NULL REFERENCES players(user_num) ON DELETE CASCADE,
    season_id       SMALLINT    NOT NULL,
    ...
    CONSTRAINT uq_octagon UNIQUE (user_num, season_id, matching_mode)
);

CREATE INDEX IF NOT EXISTS idx_octagon_user ON octagon_scores (user_num);
...
JOIN players p ON p.user_num = gd.user_num;
...
COMMENT ON COLUMN game_details.matching_mode    IS '2=일반, 3=랭크';
COMMENT ON COLUMN game_details.matching_team_mode IS '1=솔로, 2=듀오, 3=스쿼드';
